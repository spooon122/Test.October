DO $$
    DECLARE
        NumberOfTrades INT := 100;
        OpenPrice FLOAT;
        OpenTime TIMESTAMP;
    BEGIN
        FOR i IN 1..NumberOfTrades LOOP
                OpenPrice := round((random() * (100 - 10) + 10)::numeric, 2);

                OpenTime := NOW() - INTERVAL '1 hour' * (random() * (24 - 8) + 8);

                INSERT INTO trades_table ("Ticker", "Side", "OpenPrice", "ClosePrice", "Quantity", "OpenTime", "CloseTime")
                VALUES (
                           (ARRAY['Ticker1', 'Ticker2', 'Ticker3', 'Ticker4', 'Ticker5'])[floor(random() * 5 + 1)],
